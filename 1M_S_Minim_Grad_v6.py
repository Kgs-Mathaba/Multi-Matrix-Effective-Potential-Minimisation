...
"""
v_2_true=np.zeros((omega_size,omega_size),dtype=complex)
for i in range (omega_size):
    for j in range (omega_size):
        v_2_true[i,j]=np.power(N, (i-j)/2.)*v_2[i,j]

print()
print ("Full fluctuations spectrum - rescaling removed")
for x in list(np.sort(np.sqrt(linalg.eigvals(v_2_true).real))):
    print (np.format_float_scientific(x,precision=6),",")

print ()
print ("fixed no. of decimal points")
for x in list(np.sort(np.sqrt(np.absolute(linalg.eigvals(v_2_true).real)))):
    print ("{:.8f}". format(x))
"""
...
for i in range (max_size):
    print ("Gradient wrt loops", i+1,",=,",format(pot_grad_1[i].real, '.6f'))#round (loop[i].real,5)
"""
...
"""
print ()
print ("Shape of V_2_all", V_2_all.shape)


v_2_all=multi_dot ([omega,V_2_all])/4.0

print()
print ("Full fluctuations spectrum for ALL loops")
for x in list(np.sort(np.sqrt(linalg.eigvals(v_2_all).real))):
    print (np.format_float_scientific(x,precision=6),",")

print ()
print ("fixed no. of decimal points")
for x in list(np.sort(np.sqrt(np.absolute(linalg.eigvals(v_2_all).real)))):   # absolute??
    print ("{:.8f}". format(x))
"""



""" Hessian has be to revisited
print()
print ("Hessian")
for x in list(np.sort(np.sqrt(linalg.eigvals(linalg.inv(Energy.hess_inv))*np.sqrt(2)).real*N/2.0)):
    #print (np.format_float_scientific(x,precision=6),",")
    print ("{:.8f}". format(x))
"""
...
print ("shape of pot_grad_1", pot_grad_1.shape)
print ("shape of grad_2_list[1:max_size+1,:]", grad_2_list[1:max_size+1,:].shape)
...
TString = 1 #input ("Enter inverse temperature - beta  :    ")
B = float(TString)
print ("Inverse temperature :", B)
print()

print ()
print ("Use master variables spectrum")
spectrum = list(np.sort(np.sqrt(np.absolute(linalg.eigvals(multi_dot([sub_grad_list.T,V_2_all,sub_grad_list])).real)))*N/2.0)[-omega_size:]
print (spectrum)
print()
print ("Partition function")
print (spectrum)
#print (np.multiply(-B,spectrum))
print (np.exp(np.multiply(-B,spectrum)))
print ("Z = ", np.sum(np.exp(np.multiply(-B,spectrum))))
print()
print ("Free energy")
Z=np.sum(np.exp(np.multiply(-B,spectrum)))
F=-np.log(Z)/B
print ("F =", F)

print ()
print ("Energy")
#print (spectrum * np.exp(np.multiply(-B,spectrum)))
E=np.sum(spectrum * np.exp(np.multiply(-B,spectrum)))/Z
print ("E =", E)
...
print ()
print ("'Microscopic' entropy check")
print (-np.sum(np.multiply (1/Z,np.exp(np.multiply(-B,spectrum))) * np.log(np.multiply (1/Z,np.exp(np.multiply(-B,spectrum))))))

print ()
print ("Use master variables and diagonal term spectrum")
spectrum = list(np.sort(np.sqrt(np.absolute(linalg.eigvals(multi_dot([sub_grad_list.T, V_2_all,sub_grad_list])+matrix_diag_eigenv).real)))*N/2.0)[-omega_size:]
print (spectrum)
